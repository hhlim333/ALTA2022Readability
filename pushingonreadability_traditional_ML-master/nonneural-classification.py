...
class ClassificationModel:
    ...
    def test(self, X, Y,df_test,dfs):
        ...
        print(pred)
        print(Y)
        adjacent_accuracy = np.sum(np.abs(pred - Y) <= 1) / len(pred)
        ...
        df_test['FinalLabel']=df_test['label']==df_test['prediction']
        dfs=dfs.append(df_test)
        #print(f"Test F1-score: {f1 * 100:.2f}%")
        #print(f"Test Precision: {pre * 100:.2f}%")
        #print(f"Test Recall: {rec * 100:.2f}%")
        #print(f"Test QWK: {qwk * 100:.2f}%")
        return adjacent_accuracy,acc,f1,pre,rec,qwk,dfs
...
def runner(args,k_fold,dataset,neural_model_to_use,this_handcrafted_features_list,dfs):
    #train
    df_train = pd.read_csv('result/'+str(dataset)+'.'+str(k_fold)+'.train.combined.csv')
    
    #test
    df_test = pd.read_csv('result/'+str(dataset)+'.'+str(k_fold)+'.test.combined.csv')
    ...
    for model in MODELS:
        ...
        adjacent_accuracy,acc,f1,pre,rec,qwk,dfs = model.test(X_test, Y_test,df_test,dfs)
        return adjacent_accuracy,acc,f1,pre,rec,qwk,dfs,name.replace('_', ' ').upper()
...
if __name__ == '__main__':
    ...
    neural_model_to_use = ["chinese-macbert-large"]
    ...
    handcrafted_features_to_use = [HF.FeatureSet_Total2_selection()]
    total_acc_list =[]
    for this_handcrafted_features_list in handcrafted_features_to_use:
        ...
        if dataset == 'mainland':
            k_fold = [0,1,2,3,4]
            for fold in k_fold:
                adjacent_accuracy,acc,f1,pre,rec,qwk,finalDfs,name = runner(args,fold,'mainland',neural_model_to_use,this_handcrafted_features_list,finalDfs)
            ...
            finalDfs.to_csv('result/'+str(dataset)+'.'+str(name)+'.test.combined.csv')
